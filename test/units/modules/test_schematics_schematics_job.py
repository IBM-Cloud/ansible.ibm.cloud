...
from ansible.modules.cloud.ibm import schematics_schematics_job
...
class TestJobModule(ModuleTestCase):
    ...
    def test_read_schematics_job_failed(self):
        """Test the inner "read" path in this module with a server error response."""

        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.get_job')
        mock = patcher.start()
        ...
        with self.assertRaises(AnsibleFailJson) as result:
            ...
            schematics_schematics_job.main()
    ...
    def test_create_schematics_job_success(self):
        ...
        job_status_workitem_model = dict(
        ...
        job_status_template_model = dict(
        ...
        job_status_workspace_model = dict(
            workspace_name='testString',
            status_code='job_pending',
            status_message='testString',
        ...
        job_status_schematics_resources_model = dict(
            status_code='job_pending',
            status_message='testString',
        ...
        inventory_resource_record_model = dict(
            name='testString',
            description='testString',
            location='us-south',
        ...
        job_data_work_item_last_job_model = dict(
        ...
        job_log_summary_repo_download_job_model = dict(
        )

        job_log_summary_workspace_job_model = dict(
        )

        job_log_summary_workitems_model = dict(
            workspace_id='testString',
            job_id='testString',
            log_url='testString',
        )

        job_log_summary_flow_job_model = dict(
            workitems=[job_log_summary_workitems_model],
        )

        job_log_summary_action_job_recap_model = dict(
            target=['testString'],
            ok=72.5,
            changed=72.5,
            failed=72.5,
        ...
        job_log_summary_action_job_model = dict(
            recap=job_log_summary_action_job_recap_model,
        )

        job_log_summary_system_job_model = dict(
            success=72.5,
            failed=72.5,
        )

        job_log_summary_model = dict(
            job_type='repo_download_job',
        ...
        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.create_job')
        mock = patcher.start()
        mock.return_value = DetailedResponseMock(resource)

        get_job_patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.get_job')
    ...
    def test_create_schematics_job_failed(self):
        ...
        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.create_job')
        mock = patcher.start()
        mock.side_effect = ApiException(400, message='Create schematics_job error')
        ...
        assert result.exception.args[0]['msg'] == 'Create schematics_job error'
    ...
    def test_update_schematics_job_success(self):
        ...
        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.update_job')
        mock = patcher.start()
        mock.return_value = DetailedResponseMock(resource)
    ...
    def test_update_schematics_job_failed(self):
        ...
        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.update_job')
        mock = patcher.start()
        mock.side_effect = ApiException(400, message='Update schematics_job error')
        ...
        assert result.exception.args[0]['msg'] == 'Update schematics_job error'
    ...
    def test_delete_schematics_job_success(self):
        """Test the "delete" path - successfull."""
        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.delete_job')
        mock = patcher.start()
    ...
    def test_delete_schematics_job_not_exists(self):
    ...
    def test_delete_schematics_job_failed(self):
        """Test the "delete" path - failed."""
        patcher = patch('ansible.modules.cloud.ibm.schematics_schematics_job.SchematicsV1.delete_job')
        mock = patcher.start()
        mock.side_effect = ApiException(400, message='Delete schematics_job error')
        ...
        assert result.exception.args[0]['msg'] == 'Delete schematics_job error'
